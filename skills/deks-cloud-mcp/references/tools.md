...
- `delete_presentation(presentation_id, expected_revision, confirmation_name)` — prepare, but do not execute, permanent deletion of one exact presentation. Call only after the user chooses exactly one name and explicitly requests permanent deletion. Re-read that presentation immediately before calling and pass its fresh revision and exact current name. The read-only result reports `confirmationRequired: true`, a unique `confirmationId`, and renders `ui://deks/confirm-presentation-deletion-v4.html`; the presentation must still exist at this point. The same confirmation ID binds the structured result to the signed, expiring confirmation token that exists only in result `_meta` for the widget. The card remains disabled if those IDs do not match. Never request, copy or expose the token to the model.
