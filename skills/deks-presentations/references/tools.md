...
- `render_slide_preview(presentation_id, slide_id, expected_revision, width?)` — render one private checkpoint as a PNG (`width` is `1280` or `1600`). Always pass the freshly read revision; the image is sent to the MCP client/model and is the basis for rendered QA, not a data-isolation mechanism.
- `validate_layout(presentation_id)` — return geometry-only errors and AABB collision/outside-canvas warnings. It is not rendered visual QA and currently has no semantic understanding of containment or z-order intent.
...
At package release time, verify capability boundaries against the MCP discovery result. The current server exposes rendered slide previews, offline icon discovery, and palette recommendations. Asset upload, `.deks` import, PPTX export, native speaker notes, and source metadata remain web-app or future-server workflows. Use visible text for source notes and `export_deck` for portable `.deks` output when the discovered server exposes it.
